...
namespace AdvancedLightGenerationOptions
{
    internal static class EventGenerator
    {
        ...
        {
            int min = cfg.MinLaserSpeed;
            int max = cfg.MaxLaserSpeed;
            if (min > max) min = max;
            ...
            // map qb value (0-1) to speed range scaled by config multiplier
            float range = (max - min) + 1;
            return (int)Math.Max(min, Math.Min(max, Math.Round(qb.Values[qbIndex] * range * cfg.LaserSpeedMulti + (min - 1))));
        }
        ...
        private static List<BaseEvent> GenerateLaserLightEvents(List<List<float>> left, List<List<float>> right, AutoLightV2Config cfg, QBBrightness qb, Random rnd)
        {
            ...
            foreach (var tuple in new[] { new { et = 2, side = left }, new { et = 3, side = right } })
            {
                ...
                foreach (var iv in tuple.side)
                {
                    ...
                    var laserSpeedEvent = new BaseEvent { JsonTime = iv[0], Type = 10 + tuple.et, Value = GetLaserSpeed(iv[0], qb, cfg), FloatValue = 1 }; // start speed
                    laserSpeedEvent.CustomDirection = rnd.Next(0, 2); // Random direction 0 or 1
                    laserSpeedEvent.CustomLockRotation = false;
                    laserSpeedEvent.WriteCustom(); // Ensure custom data is initialized
                    events.Add(laserSpeedEvent);
                    ...
                    for (int i = 1; i < iv.Count - 1; i++)
                    {
                        ...
                        {
                            var speedResetEvent = new BaseEvent { JsonTime = iv[i], Type = 10 + tuple.et, Value = GetLaserSpeed(iv[i], qb, cfg), FloatValue = 1 }; // speed reset
                            speedResetEvent.CustomDirection = rnd.Next(0, 2); // Random direction 0 or 1
                            speedResetEvent.CustomLockRotation = false;
                            speedResetEvent.WriteCustom(); // Ensure custom data is initialized
                            events.Add(speedResetEvent);
                        }
        ...
        private static List<BaseEvent> GenerateRotationEvents(List<BaseNote> notes, AutoLightV2Config cfg, QBBrightness qb, List<BaseBookmark> bookmarks)
        {
            ...
            int min = cfg.MinRotation;
            int max = cfg.MaxRotation;
            if (min > max) min = max;
            var rnd = new Random(0);
            for (var b = AlignToBar(first, bookmarks); b < last; b += cfg.RotationInterval)
            {
                if (qb != null && cfg.DoubleAtIntenseSections)
                {
                    ...
                    {
                        var midRotationEvent = new BaseEvent { JsonTime = b + cfg.RotationInterval / 2f, Type = 8, Value = GetRotationSpeed(qb, idx, min, max), FloatValue = 1f };
                        AddRotationCustomData(midRotationEvent, rnd, cfg);
                        events.Add(midRotationEvent);
                    }
                }

                var mainIdx = Math.Max(0, Math.Min(qb.Values.Count - 1, (int)Math.Round(b * 4) - qb.StartQB));
                var mainRotationEvent = new BaseEvent { JsonTime = b, Type = 8, Value = GetRotationSpeed(qb, mainIdx, min, max), FloatValue = 1f };
                AddRotationCustomData(mainRotationEvent, rnd, cfg);
                events.Add(mainRotationEvent);
            }

            return events;
        }

        private static void AddRotationCustomData(BaseEvent evt, Random rnd, AutoLightV2Config cfg)
        {
            evt.CustomRingRotation = (float)(rnd.NextDouble() * (cfg.MaxRotation - cfg.MinRotation) + cfg.MinRotation);
            evt.CustomStep = (float)(rnd.NextDouble() * (cfg.MaxRotationStep - cfg.MinRotationStep) + cfg.MinRotationStep);
            evt.CustomProp = (float)(rnd.NextDouble() * (cfg.MaxRotationProp - cfg.MinRotationProp) + cfg.MinRotationProp);
            evt.CustomSpeed = (float)(rnd.NextDouble() * (cfg.MaxRotationSpeed - cfg.MinRotationSpeed) + cfg.MinRotationSpeed);
            evt.CustomDirection = rnd.Next(0, 2); // Random direction 0 or 1
            evt.WriteCustom(); // Ensure custom data is initialized
        }

        private static int GetRotationSpeed(QBBrightness qb, int idx, int min, int max)
        {
            if (qb?.Values == null || qb.Values.Count == 0)
                return min;
            float range = (max - min) + 1;
            return (int)Math.Max(min, Math.Min(max, Math.Round(qb.Values[idx] * range + (min - 1))));
        }
        ...
        private static List<BaseEvent> GenerateZoomEvents(List<BaseNote> notes, AutoLightV2Config cfg, QBBrightness qb, List<BaseBookmark> bookmarks)
        {
            ...
            var last = notes.Last().JsonTime;
            const float threshold = 0.5f;
            int min = cfg.MinZoomSpeed;
            int max = cfg.MaxZoomSpeed;
            if (min > max) min = max;
            var rnd = new Random(0);
            for (var b = AlignToBar(first, bookmarks); b < last; b += cfg.ZoomInterval)
            {
                if (qb != null && cfg.DoubleAtIntenseSections)
                {
                    ...
                    {
                        var midZoomEvent = new BaseEvent { JsonTime = b + cfg.ZoomInterval / 2f, Type = 9, Value = GetZoomSpeed(qb, idx, min, max), FloatValue = 1f };
                        AddZoomCustomData(midZoomEvent, rnd, cfg);
                        events.Add(midZoomEvent);
                    }
                }

                var mainIdx = Math.Max(0, Math.Min(qb.Values.Count - 1, (int)Math.Round(b * 4) - qb.StartQB));
                var mainZoomEvent = new BaseEvent { JsonTime = b, Type = 9, Value = GetZoomSpeed(qb, mainIdx, min, max), FloatValue = 1f };
                AddZoomCustomData(mainZoomEvent, rnd, cfg);
                events.Add(mainZoomEvent);
            }

            return events;
        }

        private static void AddZoomCustomData(BaseEvent evt, Random rnd, AutoLightV2Config cfg)
        {
            evt.CustomStep = (float)(rnd.NextDouble() * (cfg.MaxZoomStep - cfg.MinZoomStep) + cfg.MinZoomStep);
            evt.CustomSpeed = (float)(rnd.NextDouble() * (cfg.MaxZoomSpeed - cfg.MinZoomSpeed) + cfg.MinZoomSpeed);
            evt.WriteCustom(); // Ensure custom data is initialized
        }

        private static int GetZoomSpeed(QBBrightness qb, int idx, int min, int max)
        {
            if (qb?.Values == null || qb.Values.Count == 0)
                return min;
            float range = (max - min) + 1;
